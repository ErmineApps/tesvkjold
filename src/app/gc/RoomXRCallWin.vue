...
export default {
  name: "RoomXRCallWin",
  components: {XrPreviewScreenshots, DevPanel, AudioTranslation, SaveVideoSession, RoomXRScreenSharing, Confirm},
  props: {
 ...
 isPanelShowOptions: false,
  ...
  methods: {
    ...
    // region Добавление нового видео-----receiveVideoFromAudioError--------------------------------------------------------------------------------
...
<!--                          <span <div id="${'spanNoVideo' + senderId}" class="fa fas fa-video-slash v-btn_ot_stats_sp ma-2" style="color: white; font-size: 60px" aria-hidden="true"></span>-->
      ...
      if (!this.isFirefox) {
        ...
        // var videoDops = document.getElementById('devDevBlockVideoInfo_' + senderId);
        //
        // videoDops.onmouseover = videoDops.onmouseout = videoDops.onmousemove = this.handlerPS();
        //
        //
        // videoDops.addEventListener("mouseout", function (event) {
        //
        //   This.onOut(senderId)
        // }, false);
        //
        //
        // videoDops.addEventListener("mouseover", function (e) {
        //
        //   This.onIn(senderId, participant.OperFunc)
        // }, false);
        //
        ...
        //   This.onIn(senderId, participant.OperFunc)
        // });
        // btnBlockOperAT.on('click', function () {
        //   alert('clickAT')
        // });
      }
    ...
    offerToReceiveVideo(context,
      ...
      //
      // if(numSession > 8) {
      //   console.log('Снижение качества видео до 240')
      //   var participant = stream.calls[This.senderId].peer
      //   var media_streams = participant.peerConnection.getLocalStreams();
      //   var video_tracks = media_streams[0].getVideoTracks();
      //   video_tracks[0].applyConstraints({
      ...
      //   console.log('Снижение качества видео до 320')
      //     var participant = stream.calls[This.senderId].peer
      //     var media_streams = participant.peerConnection.getLocalStreams();
      //     var video_tracks = media_streams[0].getVideoTracks();
      //     video_tracks[0].applyConstraints({
      ...
      //     });
      //   }else{
      //   console.log('Без снижения качества видео')
      // }
      ...
      if (roundTripTime > 40) {
        if (this.numHighNet < 5) {
          ...
          if (!this.isVideoHardNet) {
            ...
            //     This.isVideoUser(participant.senderId, false, true, true, false)
            //   } else {
            //     This.closeMyVideoPS()
            //   }
            // }
        ...
        if (this.numHighNet > 1) {
          ...
          if (this.isVideoHardNet) {
            ...
            //     This.isVideoUser(participant.senderId, false, true, true, true)
            //   } else {
            //
            //   }
            // }
...
// если подключили камеру без доступа
                  ...
                  if (sender.track.kind == 'video') {
                    ...
                    if (STREAM_VIDEO != null) {
                      ...
                      const constraints = {
                        ...
                        frameRite: 10,
                        advanced: [
                          {width: 320, height: 240, frameRite: 10},
                          {aspectRatio: 1.333}
                        ]
                      };
    ...
    stopSaveCall() {
      ...
      // console.log('!!!! stopSaveCall stopRecording')
      // recorderTest.stopRecording(function() {
    ...
    testREC() {
      // // var elementToRecord = document.getElementById('router-view');
      // var elementToRecord = document.getElementById('mainRoom');
      ...
      //
      //
      // recorderTest = new Record(elementToRecord, {
      //   type: 'canvas'
      // });
      //
...
<!--                          <span <div id="${'spanNoVideo' + senderIdtest}" class="fa fas fa-video-slash v-btn_ot_stats_sp ma-2" style="color: white; font-size: 60px" aria-hidden="true"></span>-->
  ...
  <!--                           <span class="fa fas fa-microphone-alt v-btn_ot_stats_sp ma-2" aria-hidden="true"></span>-->
    ...
    saveScreenshot(senderId) {
      ...
      // var canv = stream.calls[senderId].peer.currentFrame;
      // console.log('ss canv', canv)
      //
      // setTimeout(function () {
      //   if (!canv) {
      //     return
      ...
      //         console.log('ss reader', reader.result)
      //         This.sendScreenshot(reader.result, senderId)
      //       }
      //     }, 'image/jpeg', 1)
      //   } else if (canv.msToBlob) {
      //     var blob = canv.msToBlob()
      //     //console.log(blob)
      //     var reader = new FileReader()
      //     reader.readAsDataURL(blob)
      //     reader.onloadend = function () {
      //       console.log('ss ms reader', reader.result)
      //       This.sendScreenshot(reader.result, senderId)
      //     }
      //   } else {
      //     This.$root.showErr('Скриншоты видео не поддерживаются Вашим браузером')
      //   }
      // }, 0)
