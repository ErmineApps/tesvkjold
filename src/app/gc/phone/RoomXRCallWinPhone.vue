<template>
  <div>
    <div v-if="mainWin>0" class="div-mainwinPh">
      <p class="p-mainwinPh">{{ textFuncOther }}</p>
    </div>
 ...
 padding: 7px;
    ...
    <v-footer
      ...
      :absolute=true
      :padless=true class="vFooterRoom"
    ...
    >
      ...
      >
        <v-card-text>
          ...
          <v-btn :title="isStreamMic ? $t('gc.func.disable-mic') : $t('gc.func.play-mic')" class="mr-1 btnFooter1"
                 color="white"
                 fab
                 @click="setIsMic">
            <v-icon v-if="isStreamMic" style="font-size: 16px;">fas fa-microphone-alt</v-icon>
            <v-icon v-if="!isStreamMic" style="font-size: 16px;">fas fa-microphone-alt-slash</v-icon>
          </v-btn>
          ...
          <v-btn v-if="!isPhone" :title="$t('gc.more')" class="mr-1 btnFooter2"
                 color="white"
                 fab
                 @click="setIsMoreWin">
          ...
          <v-btn v-if="isDevGC" :title="$t('gc.dev.btn')" class="mr-1 btnFooter3 devBtnGC"
                 color="white"
                 fab
                 @click="addWinTest">
            <v-icon style="font-size: 16px; color: #ff6600">fas fa-user-plus</v-icon>
          </v-btn>

          <v-btn v-if="!isXrOper" :title="$t('gc.func.close-call')" class="mr-1 btnFooter3"
                 color="red"
                 fab
                 @click="closeCall">
          ...
          <v-row class="vrow-menu-right">
            ...
            <v-btn v-if="devGC" class="mr-1 btnFooter2" color="white"
                   fab
                   @click="setNewMicADd">
            ...
            <!------------------------------------------------------------->
            <!-- Текстовые сообщения -->
            <v-btn v-if="!isPhone" :title="$t('gc.chat')" class="mr-1 btnFooter2"
                   color="white"
                   fab
            ...
            <v-btn v-if="!isPhone && numUsers>1"
              ...
              <v-row v-if="isTypeWin" class="rowIconLayout">
                <v-btn v-if="!isPhone && numUsers>1 && typeMaket != 3" :title="maketvideo3" class="mr-1 btnFooter2"
                       color="white"
                       fab
                       @click="setTypeWinPH(3, false, true, true)">
                  <img :src="svgLayoutV3_" alt="" style="width: 34px; height: 22px;">
                </v-btn>
                <v-btn v-if="!isPhone && numUsers>1 && typeMaket != 2" :title="maketvideo2" class="mr-1 btnFooter2"
                       color="white"
                       fab
                       @click="setTypeWinPH(2, false, true, true)">
                  <img :src="svgLayoutV1_" alt="" style="width: 34px; height: 22px;">
                </v-btn>
                <v-btn v-if="!isPhone && numUsers>1  && typeMaket != 1" :title="maketvideo1" class="mr-1 btnFooter2"
                       color="white"
                       fab
                       @click="setTypeWinPH(1, false, true, true)">
    ...
    <SaveVideoSession ref="refSaveVideo" :audioOutputValue="audioOutputValue" :hasCamera="hasCamera"
                      :lang="paramValueLang"
                      :wsURL="wsURL" @saveClose="saveClose"
                      @saveSecondClose="saveSecondClose" @startSave="startSave"></SaveVideoSession>

    <Confirm ref="refConfirm" @agree="confirmAgree" @agreeSS="closeSSAndScrS" @cancel="confirmCancel"></Confirm>

    <div id="idDivCanvasOther">
      <canvas id="idCanvasPrintOther" :height="hCanvas"
              :width="wCanvas" class="canvasPrintOther"></canvas>
    </div>
...
import ringToneNew from '@/assets/mp3/newCall.mp3';
import ringToneDelete from '@/assets/mp3/deleteCall.mp3';
...
export default {
  name: "RoomXRCallWinPhone",
  components: {AudioTranslation, SaveVideoSession, RoomXRScreenSharing, Confirm},
  props: {
  ...
  data() {
    return {
      ...
      typeMaket: 3, // тип макета 1 - много, 2 - одно большое, 3 - много с моим видео
      typeMaketOld: 1,
      isTypeWin: false, // отображение выбора макета

      audioNewCall: new Audio(ringToneNew),
      audioDeleteCall: new Audio(ringToneDelete),
      ...
      oldsetTypeWinPH: 0, //This.setTypeWinPH(This.oldsetTypeWinPH, false, true)
  ...
  watch: {
    ...
    isOrientland: {
      handler: function () {
        if(This.typeMaket != 2){
          This.setTypeWinPH(3, false, true, false) //смена ориентации
        }
  ...
  methods: {
    ...
    init(_dev, _ws, _senderName, _hasCamera, _typeBrowser, _isTranslateFunc, _isDevGC, _videoValue, _audioInputValue, _audioOutputValue, _S_OLD_TREAM_VIDEO_FON, _STREAM_AUDIO, serverUrl, isDevUrl) {
      console.log('RoomXRCallWin: init(_dev, _ws, _senderName, _hasCamera, _typeBrowser, _isTranslateFunc, _isDevGC, _videoValue, _audioInputValue, _audioOutputValue, _S_OLD_TREAM_VIDEO_FON, _STREAM_AUDIO, serverUrl, isDevUrl, isDevGC)',
        _dev, _ws, _senderName, _hasCamera, _typeBrowser, _isTranslateFunc, _isDevGC, _videoValue, _audioInputValue, _audioOutputValue, _S_OLD_TREAM_VIDEO_FON, _STREAM_AUDIO, serverUrl, isDevUrl)
      ...
      $('#divMainLinkUser').removeClass('divMainLink')
      $('#divMainLinkUser').addClass('divMainLinkPh')
      ...
      if (!STREAM_AUDIO) {
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
            ...
            // tmp
            console.log('navigator.mediaDevices', navigator.mediaDevices, navigator.mediaDevices.getUserMedia({audio: true}))
      ...
      // пока времено отключил, для тестов
      var oldData = []

      // setTimeout(function run() {
      //   // console.log('__L__________________________________________________________________')
      //   // console.log('__O__________________________________________________________________')
      //   // console.log('__G__________________________________________________________________')
      //
      //   var keys = Object.keys(stream.calls);
      //   // console.log('всего участников '+keys.length)
      //
      //   for (var i = 0; i < keys.length; ++i) {
      //     var participant = stream.calls['' + keys[i]];
      //
      //     if (participant && participant.peer && !This.isFirefox) {
      //
      //       // console.log('статистика webrtc от '+keys[i])
      //
      //       participant.peer.peerConnection.getStats().then(function (reports) {
      //         // console.log(reports)
      //         reports.forEach(function (stat) {
      //           // console.log('stat___ ', stat)
      //           if (stat.id === 'RTCPeerConnection'){
      //            // console.log('Тип RTCPeerConnection: '+stat.type)
      //           }
      //           else if(stat.type === 'remote-candidate'){
      //             // console.log('remote-candidate, port = '+stat.port+' '+stat.protocol+'   '+stat.ip)
      //           }
      //         })
      //
      //         setTimeout(()=>{
      ...
      //                         for (var i = 0; i < keys.length; ++i) {
      //                           var participant = stream.calls['' + keys[i]];
      //                           if(participant.idTrackVideo == statTrack.id) {
      //                             $('#devDevBlockConnect_' + keys[i]).remove()
      //                           }
      //                         }
      //                       }else{
      //                         let d = new Date;
      //                         let ms = d.getTime();
      //                         console.log(ms); // 709
      //                         console.error('Ошибка track video, id = '+statTrack.id+'ended '+ statTrack.ended +' framesReceived = ' + statTrack.framesReceived + ' frameWidth = '+statTrack.frameWidth+', задержка пакетов '+jus)
      //                         var keys = Object.keys(stream.calls);
      //                         for (var i = 0; i < keys.length; ++i) {
      //                           var participant = stream.calls['' + keys[i]];
      //                           if(participant.idTrackVideo == statTrack.id) {
      //                             let d2 = new Date;
      //                             var delta = d.getTime() - participant.dateConnect
      //                             if (delta > 10000) {
      //                               $('#devDevBlockConnect_' + keys[i]).hide()
      //                               this.$root.showInfo(This.$t('reject.callinfo.error1')+keys[i]+This.$t('reject.callinfo.error2'))
      //                               console.error('Проблемы с подключением к ' + keys[i] + ' пробуем переподключиться')
      //                               This.reconnectUser(keys[i])
      //                             } else {
      //                               console.log(keys[i] + ' ожидаем подключения')
      //                             }
      //                           }
      //                         }
      //                       }
    ...
    onNewParticipant(request) {
      console.log('onNewParticipant request, request.hasCamera')
      console.log(request, request.hasCamera)
      let __hasCamera;
      ...
      this.receiveVideo(request.senderName, request.bodyPix, request.isMic,
        request.isVideo,
        false, false, __hasCamera, request.avatar, request.senderId, request.orient, request.delta);
    },
    ...
    onExistingParticipants(msg, _isStartMic, _isStartVideo) {
      ...
      var constraints = {
        audio: {optional: [{sourceId: audioSource}, {echoCancellation: true}, {noiseSuppression: true}, {autoGainControl: true}, {volume: 1.5}]}, // ??
      ...
      $('#imgDevIconNoVideoMain').hide()
      $('#noImgDevIconNoVideoMain').hide()
      $('#imgDevIconNoVideoMain1').hide()
      $('#noImgDevIconNoVideoMain1').hide()

      //console.log('del 2 senderName = '+this.senderName)
      var nameUC = this.setNameUC(this.senderName)

      var videoAdd = `<div id="${'devVideo_' + this.senderId}" class="winVideoRoom1">

                            <div id="${'devDevIconNoVideoMain'}" class="devDevIconNoVideoPh">
                              ...
                              <img id="${'imgDevIconNoVideoMain'}" class="imgDevIconNoVideoTextPhDev">
                              <div id="${'noImgDevIconNoVideoMain'}" class="devDevIconNoVideoTextDev">${nameUC}</div>
                          </div>`
      $('#flexParticipants').append(videoAdd);
      ...
      msg.data.forEach(function (item, i, arr) {
        console.log('This.receiveVideo(item.senderName, item.isBodyPix, item.isMIc, item.isVideo, item.isFuncOper, item.isOperFuncOper, item.avatar, item.senderId, item.orient, item.delta)',
          item.senderName, item.isBodyPix, item.isMIc, item.isVideo, item.isFuncOper, item.isOperFuncOper, item.hasCamera, item.avatar, item.senderId, item.orient, item.delta)
        ...
        setTimeout(()=>{This.setTypeWinPH(3, false, true, false)},0)

        console.log('item.avatar ' + urlRest + item.avatar)
        This.$root.restGet(urlRest + item.avatar).then((res) => {
          console.log('getAvatarGC res', res)
          This.receiveVideo(item.senderName, item.isBodyPix, item.isMIc, item.isVideo, item.isFuncOper, item.isOperFuncOper, __hasCamera, res.avatar, item.senderId, item.orient, item.delta)
        })
      });
      This.resizeWin(3)
      // endregion
    ...
    onOfferCall(context, error, offerSdp, senderId) {
      ...
      console.log('Подключаемся к группе onOfferCall ', context)
      ...
      try {
        ...
        console.log("onOfferCall error ", e)
        this.$root.showInfo('Ошибка формирования offerSdp. Передача и получение не доступно')
      }
      ////console.log('offerSdp ___ ' + offerSdp)
    ...
    //item.name, item.isBodyPix, item.isMIc, item.isVideo, item.isFuncOper, item.isOperFuncOper, item.avatar
    receiveVideo(senderName, isBodyPix, isMic, isVideo, isFuncOper, isOperFuncOper, hasCamera, avatar, senderId, orient, delta) {
      ...
      if (senderName.substring(0, 9) == 'Оператор_' || senderName.substring(0, 9) == 'Operator_') {
        ...
        if (This.isAddToAll) {
          ...
          // //$('#idDivLocalVideo').removeClass('iddivlocalvideoPh2')
          // //$('#idDivLocalVideo').addClass('iddivlocalvideoPh')
      ...
      var video12 = `<div id="${'devVideo_' + senderId}" class="winVideoRoom1">
                          <div id="${'devDevIconNoVideo_' + senderId}" class="devDevIconNoVideoPh">

                            <img id="${'imgDevIconNoVideo_' + senderId}" class="imgDevIconNoVideoTextPhDev">
...
<!--                          <span <div id="${'spanNoVideo' + senderId}" class="fa fas fa-video-slash v-btn_ot_stats_sp ma-2" style="color: white; font-size: 60px" aria-hidden="true"></span>-->
                        ...
                        <div id="${'devDevBlockVideoInfo_' + senderId}">
                          ...
                          <div class="winVideoRoomInfo row">
                             ...
                             <span id="${'minBitrateVideo_' + senderId}" class="fa fas fa-signal v-btn_ot_stats_sp ma-2 infoSpanWin2" style="color: red" aria-hidden="true"></span>
      ...
      $('#flexParticipants').append(video12);
      $('#flexParticipants').append(audio12);
      ...
      if (participant.scrs) {
        ...
        setTimeout(()=>{
          This.mainVideoPosition(senderId) //res
          This.isScrSvision=true
        }, 1000)
      ...
      //       This.setTypeWinPH(2, true)//нажатие на окно оператора
      //       console.log('setTypeWinPH 2 нажатие на окно оператора')
      //       setTimeout(() => {
      ...
      //       This.isSoundWinFunc = false
      //       stream.calls[senderId].full = true
      //       This.setTypeWinPH(2, true)//devDevBlockVideoInfo_
      //       setTimeout(() => {
      //         if (This.numScreenSharinWin < 1) {
      //           This.isSoundWinFunc = true
      //         }
      ...
      //   });
      //   // var btnBlockOperAT = $('button#devBtnOperFuncAT_' + senderId)
      //   // btnBlockOperAT.mouseover(e => {
      //   //   This.onIn(senderId, participant.OperFunc)
      //   //
      //   //   $('#iconOperFuncAT1_' + senderId).hide()
      //   //   $('#iconOperFuncAT2_' + senderId).show()
      //   //   $('#iconOperFuncAT3_' + senderId).hide()
      //   // });
      //   // btnBlockOperAT.on('mouseup', function () {
      //   //
      //   //   $('#iconOperFuncAT1_' + senderId).hide()
      //   //   $('#iconOperFuncAT2_' + senderId).show()
      //   //   $('#iconOperFuncAT3_' + senderId).hide()
      //   //   This.sendAudioATOper(senderId)
      //   // });
      //   // btnBlockOperAT.on('mousedown', function () {
      //   //
      //   //   $('#iconOperFuncAT1_' + senderId).hide()
      //   //   $('#iconOperFuncAT2_' + senderId).hide()
      //   //   $('#iconOperFuncAT3_' + senderId).show()
      //   //   This.saveAudioATOper(senderId, senderName)
      //   // });
      //   // btnBlockOperAT.on('mouseout', function () {
      //   //
      //   //   $('#iconOperFuncAT1_' + senderId).show()
      //   //   $('#iconOperFuncAT2_' + senderId).hide()
      //   //   $('#iconOperFuncAT3_' + senderId).hide()
      //   // });
      // }
    ...
    offerToReceiveVideo(context,
      ...
      // if (stream.calls[senderId].senderName.substring(0, 9) == 'Оператор_'||stream.calls[senderId].senderName.substring(0, 9) == 'Operator_') {
      //   setTimeout(()=>{
      //
      //     let sendersTr = stream.calls[senderId].peer.peerConnection.getReceivers();
      //     console.log('sendersTr',sendersTr)
      //     sendersTr.forEach((sender) => {
      //       try {
      ...
      //   try{
      //     var participant = stream.calls[name].peer
      //     if(participant){
      //       if(!This.isFirefox){
      //         This.fetatsChrome(participant, name)
      //       }else{
      //         This.fetchStatsFirefox(participant)
      //       }
      //     }
    ...
    onParticipantLeft(request) {
      ...
      if (request.senderName.substring(0, 7) == 'scrsXR_') {
        ...
        This.isSoundWinFunc = false
        This.isScrSvision = false
        This.updateMainOperIG(2)

        var keys = Object.keys(stream.calls);
        var numSession = keys.length
        if (numSession == 1) {
          this.typeMaket = 3
          this.setTypeWinPH(this.typeMaket, false, true)//если одна сессия  закрытие окна с расшариванием
          setTimeout(() => {
            this.resizeWin(103)
          }, 400)
        } else {
          this.setTypeWinPH(-1, false, true)// закрытие окна с расшариванием
        }

        // this.typeMaket = this.typeMaketOld
        // if (this.typeMaket == 3) {
        //   this.setTypeWinPH(this.typeMaket, false, true)
      ...
      if (participant_obj) {
        ...
        if (this.typeMaket == 2 && !this.isMyFullVideo || this.isAddToAll) {
          ...
          if (numSession == 2) {
            This.setTypeWinPH(1)//если всего 2 сессии
          } else {
            for (var i = 0; i < keys.length; ++i) {
              var participant = stream.calls['' + keys[i]];
          ...
          if (numSession == 1) {
            this.typeMaket = 3
            this.setTypeWinPH(this.typeMaket, false, true)//если одна сессия
            setTimeout(() => {
              this.resizeWin(101)
            }, 400)
          } else {
      ...
      try {
        if (senderId != undefined) {
          ...
          if (this.typeMaket == 2 && !this.isMyFullVideo || this.isAddToAll) {
            ...
            if (numSession == 2) {
              This.setTypeWinPH(1)//если 2 сессии
            } else {
              for (var i = 0; i < keys.length; ++i) {
                var participant = stream.calls['' + keys[i]];
            ...
            if (numSession == 1) {
              this.typeMaket = 3
              this.setTypeWinPH(this.typeMaket, false, true)//если 1 сессия
              setTimeout(() => {
                this.resizeWin(131)
              }, 400)
            } else {
    ...
    fetatsChrome(peer, name) {
      ...
      try {
        const stats = peer.peerConnection.getStats(null);

        stats.forEach(report => {
          if (report.type === 'inbound-rtp') {
            const reportName = report.id;
            const reportType = report.type;
            const reportSide = reportName.slice(-4);

            if ((reportSide === 'send') && (report.stat('mediaType') === 'video')) {
    ...
    powerSignal(level) {
      ...
      this.$emit('onLevelRTT', true, level)

      if (level > 400) {
        if (this.numHighNet < 3) {
          this.numHighNet++
        } else if (this.numHighNet == 3) {
          this.numHighNet++
          if (!this.isVideoHardNet) {
            // this.$root.showInfo('Режим : высокая задержка сети')
            ...
            for (var i = 0; i < keys.length; ++i) {
              ...
              var participant = stream.calls['' + keys[i]];
              if (participant.senderId != This.senderId) {
                var media_streams = participant.peer.peerConnection.getRemoteStreams();
                var video_tracks = media_streams[0].getVideoTracks();
                video_tracks[0].enabled = false;
              } else {
                This.closeMyVideoPS()
    ...
    closeMainVideo() {
      console.log('closeMainVideo убрать все закрепления окон ')
      var keys = Object.keys(stream.calls);
      for (var i = 0; i < keys.length; ++i) {
        var participant_obj = stream.calls['' + keys[i]]
        participant_obj.full = false
...
//  this.analyser.connect(this.context.destination);
    ...
    mainVideoPosition(senderId, isMain, _mainWin) {
      // //test123
      console.log('mainVideoPosition Сделать окно главным относительно остальных '+senderId)
      // document.getElementById('video_' + userName.replace(/ /g, '')).requestFullscreen()

      if (this.isShowFuncOther || this.isScrSvision) {
        console.log('mainVideoPosition Сделать окно главным, нет ',this.isShowFuncOther, this.isScrSvision)
        // this.$root.showInfo(This.$t('reject.call.info.noFunc7'))
      } else if (!this.isPaint && this.mainWin < 2 || _mainWin) {
        console.log('mainVideoPosition 1 '+senderId)
        var participant = stream.calls['' + senderId]
        ...
        if (participant.isMainWin) {
          ...
          participant.full = true
          This.setTypeWinPH(2, true)// включены функции оператора isShowFuncOther
        ...
        console.log('mainVideoPosition 2')
        if (this.isPaint) {
    ...
    newUserEcpect(text, name) {
      ...
      //     showClose: false,
      //     closeDelay: 5000,
      //     actionText: This.$t('expand'),
      //     onActionClick: ()=>{
      //       This.$notify.closeAll()
      //       This.setPanelInfoUser(true)
      //     },
      //   });
      // }
    },
    ...
    setTypeWinPH(type, _full, _click, _save) {
      ...
      if (this.isShowFuncOther) {
        ...
        if (This.isMyFullVideo) {
          ...
          // $('#idnewMyVideoClass').css('width', '100%')
          // $('.row.pa-6.text-center.grey.lighten-2.rounded-bl-xl.infoTooldev').append(//$('#idDivLocalVideo'));
          // $('#flexParticipants').remove($('#devVideo_Opermain'));
        }
      ...
      var numSession = keys.length
        ...
        if (this.typeMaket == 2) {
          ...
          for (var i = 0; i < keys.length; ++i) {
            ...
            if (participant.full && participant.senderId != This.senderId) {
              $('#participants').css('overflow-y', 'hidden')
              $('#devVideo_' + partName).addClass('winVideoRoomFullyPH')
              $('#devDevBlockVideo_' + partName).addClass('winVideoRoomDevFullyPH')
              $('#devDevBlockVideoInfo_' + partName).addClass('winVideoRoomDevFullyPH')
              try {
          ...
          $('#flexParticipants').show()
          if (this.isOrientland) {
            console.log('!!!!!! this.isOrientland Горизонт ' + this.isOrientland)

            $(`#flexParticipants`).removeClass('d-flex flex-column pl-2 justify-lg-center mb-6 bg-surface-variant flexParticipantsCss')
            $(`#flexParticipants`).addClass('d-flex justify-center align-content-space-around flex-wrap bg-surface-variant flexParticipantsCss')
            ...
            for (var i = 0; i < keys.length; ++i) {
              ...
              if (numSession > 0 && numSession < 5) {
                $('#devVideo_' + partName).addClass('winVideoRoom45PH')
                $('#devDevBlockVideo_' + partName).addClass('winVideoRoomDev45PH')
                $('#devDevBlockVideoInfo_' + partName).addClass('winVideoRoomDev45PH')
              } else if (numSession > 4 || numSession < 10) {
                $('#devVideo_' + partName).addClass('winVideoRoom67PH')
                $('#devDevBlockVideo_' + partName).addClass('winVideoRoomDev67PH')
                $('#devDevBlockVideoInfo_' + partName).addClass('winVideoRoomDev67PH')
              } else {
                $('#devVideo_' + partName).addClass('winVideoRoom89PH')
                $('#devDevBlockVideo_' + partName).addClass('winVideoRoomDev89PH')
                $('#devDevBlockVideoInfo_' + partName).addClass('winVideoRoomDev89PH')
              }
              ...
              //
              // } else {
                $('#flexParticipants').append($('#devVideo_' + partName));
              // }
            }
            $('#devMiniVideos').hide()
          } else {
            console.log('!!!!!! this.isOrientland Вертикаль ' + this.isOrientland)
            $(`#flexParticipants`).removeClass('d-flex justify-center align-content-space-around flex-wrap bg-surface-variant flexParticipantsCss')
            $(`#flexParticipants`).addClass('d-flex flex-column pl-2 justify-lg-center mb-6 bg-surface-variant flexParticipantsCss')
            ...
            for (var i = 0; i < keys.length; ++i) {
              ...
              $('#devVideo_' + partName).addClass('winVideoRoomPHport')
              $('#devDevBlockVideo_' + partName).addClass('winVideoRoomDevPHport')
              $('#devDevBlockVideoInfo_' + partName).addClass('winVideoRoomDevPHport')
    ...
    winDeleteClass(name) {
      ...
      $('#devVideo_' + name).removeClass('winVideoRoomPHport')
      $('#devVideo_' + name).removeClass('winVideoRoom45PH')
      $('#devVideo_' + name).removeClass('winVideoRoom67PH')
      $('#devVideo_' + name).removeClass('winVideoRoom89PH')
      ...
      $('#devVideo_' + name).removeClass('winVideoRoomFullyPH')
      $('#devVideo_' + name).removeClass('winVideoRoomFullnPH')
      ...
      $('#devDevBlockVideo_' + name).removeClass('winVideoRoomDevPHport')
      $('#devDevBlockVideo_' + name).removeClass('winVideoRoomDev45PH')
      $('#devDevBlockVideo_' + name).removeClass('winVideoRoomDev67PH')
      $('#devDevBlockVideo_' + name).removeClass('winVideoRoomDev89PH')
      ...
      $('#devDevBlockVideo_' + name).removeClass('winVideoRoomDevFullyPH')
      ...
      $('#devDevBlockVideoInfo_' + name).removeClass('winVideoRoomDevPHport')
      $('#devDevBlockVideoInfo_' + name).removeClass('winVideoRoomDev45PH')
      $('#devDevBlockVideoInfo_' + name).removeClass('winVideoRoomDev67PH')
      $('#devDevBlockVideoInfo_' + name).removeClass('winVideoRoomDev89PH')
      ...
      $('#devDevBlockVideoInfo_' + name).removeClass('winVideoRoomDevFullyPH')
    ...
    usersError(mess) {
      ...
      if (!this.devTestSession) {
        ...
        if (This.isStartCallT) {
          ...
          try {
            var arraytoSenderName= [];
            var arraytoSenderID= [];

            mess.usersActive.forEach(item => {
              arraytoSenderName.push(item.senderName)
              arraytoSenderID.push(item.senderId)
            })
            ...
            for (var i = 0; i < numSession; i++) {
              ...
              console.log('Set активных имен keys[i] ', keys[i])
              ...
              console.log('проверка ',keys[i]+' ',liveSession)
    ...
    isVideoUser(senderId, isVideo, hasCamera) {
      console.log('isVideoUser(senderId, isVideo, hasCamera)', senderId, isVideo, hasCamera)
      var senderName = stream.calls[senderId].senderName
      console.log('senderName',senderName)
       if(senderName.substring(0, 9) == 'Оператор_' || senderName.substring(0, 9) == 'Operator_' || senderName.substring(0, 7) == 'scrsXR_') {
           $('#devDevIconNoVideo_' + senderId).hide()
         ...
         console.log('isVideoUser participant ', participant)
...
// если подключили камеру без доступа
    ...
    clickItemMore(item) {
      ...
      if (item.type == 1) {// на весь экран
        ...
        if (this.devGC) {
          ...
          this.itemsMoreWin.removeItem({text: this.$t('gc.group.menu.6'), icon: 'fab fa-dev', type: 6})
        }
      } else if (item.type == 6) {
        This.devTestSession = true
        This.testUserWin++
        This.testReceiveVideo('testWinVideo' + This.testUserWin, 'testWinVideo' + This.testUserWin, false, false, true, false)
      } else if (item.type == 7) {
    ...
    // на весь экран TODO пока не сделал до конца, нужно еще доделать закрытие полного режима
    fullWindows() {
    ...
    saveSettingsCall(_audioInputValue, _audioOutputValue, _videoValue, _STREAM_VIDEO, _STREAM_AUDIO) {
      this.audioInputValue = _audioInputValue
      this.audioOutputValue = _audioOutputValue
      this.videoValue = _videoValue
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia(constraints)
          .then(function (stream) {
                ...
                if (sender.track.kind == 'video') {
                  ...
                  if (STREAM_VIDEO != null) {
                    ...
                    const constraints = {
                      ...
                      frameRite: 24,
                      advanced: [
                        {width: 640, height: 380, frameRite: 24},
                        {aspectRatio: 1.333}
                      ]
                    };
            ...
            //   var participant = stream.calls['' + keys[i]];
            //
            //   if (participant && participant.senderId != This.senderId) {
            //     console.log('This.attachSinkId($(\'#video_\' + participant.senderId)[0], this.audioOutputValue)', $('#video_' + participant.senderId)[0], this.audioOutputValue);
            //     This.attachSinkId($('#video_' + participant.senderId)[0], this.audioOutputValue)
            //   }
            // }
            STREAM_AUDIO = _STREAM_AUDIO
        ...
        navigator.getUserMedia(constraints,
          function (stream) {
            ...
            stream.getAudioTracks()[0].addEventListener("unmute", event => {
              //console.log('устройство микрофон вкл.')
            }, false);
            stream.getAudioTracks()[0].addEventListener("mute", event => {
              //console.log('устройство микрофон выкл')
            }, false);
            stream.getAudioTracks()[0].addEventListener("ended", event => {
              //console.log('устройство звука удалили')
              This.newAudioEnded()
            }, false);
    ...
    screenSharingClose() {
      ...
      if (numSession == 1) {
        this.typeMaket = 3
        this.setTypeWinPH(this.typeMaket, false, true)//если одна сессия закрыть расшаривание экрана свое
        setTimeout(() => {
          this.resizeWin(104)
        }, 400)
      } else {
        this.setTypeWinPH(-1)//закрыть расшаривание экрана свое
      }
    },
      ...
      if (is != this.isShowFuncOther && is == false) {
        ...
        setTimeout(() => {
          ...
          if (keys.length == 1) {
            ...
            This.setTypeWinPH(-1, false, true)//пользоватлей что они выключили функции оператора
          }
        }, 500)
    ...
    funcOperOther(message) {
      ...
      if (message != undefined) {
        if (message.status) {
          ...
          This.oldsetTypeWinPH = This.typeMaket
          ...
          setTimeout(() => {
            ...
            if (keys.length == 1) {
              ...
              This.setTypeWinPH(-1, false, true)//funcOperOther
            }
          }, 500)
    ...
    swipe(){
      ...
      function handleTouchMove(evt) {
        if ( ! xDown || ! yDown || This.isShowFuncOther) {
          return;
        }
    ...
    startFuncOperOther(senderId) {
      console.log('__________startFuncOperOther_'+senderId+'______________ ')
      This.closeMainVideo()


      setTimeout(()=>{
        var participant2 = stream.calls[senderId];
        console.log('__________startFuncOperOther_'+participant2.senderName)
        // participant2.isRealFuncOper = true
        // participant2.isMainWin = true

        if (This.mainVideoPosition(senderId, true, 1)) {
        // if (This.mainVideoPosition(senderId)) {
          setTimeout(() => {
            ...
            var frealWidth = v.videoWidth;
            var frealHeight = v.videoHeight;
            console.log('realVideo videoWidth w '+frealWidth + " h "+ frealHeight)
            var displayWidth = v.offsetWidth;
            var displayHeight = v.offsetHeight;
            console.log('realVideo offsetWidth w '+displayWidth + " h "+ displayHeight)
            var realWidth = displayWidth * (displayWidth/frealWidth);
            var realHeight = displayHeight * (displayHeight/frealHeight);
            console.log('realVideo w '+realWidth + " h "+ realHeight)

            var trealHeight = v.videoHeight
            var trealWidth = v.videoHeight * (16 / 9)

            console.log('testRealVideo  w '+trealWidth + " h "+ trealHeight)

            This.wCanvas = v.offsetWidth;
            This.hCanvas = Math.floor(This.wCanvas * (9 / 16));

            var d1 = 16 / 9;
            var d2 = v.offsetWidth / v.offsetHeight;
            console.log('d1 '+ d1 + ' d2 '+d2)

            if(d1 < d2){
              This.hCanvas = v.offsetHeight
              This.wCanvas = This.hCanvas *(v.videoWidth / v.videoHeight);
              canvas.css('left', (v.offsetWidth - This.wCanvas)/2 + 'px')
              canvas.css('top', 0)
              console.log('testRealVideo  w '+trealWidth + " h "+ trealHeight)
              console.log('УЖе This.hCanvasThis.wCanvas  w '+This.wCanvas  + " h "+ This.hCanvas)
            }else{
              canvas.css('top', 'calc(50% - ' + This.hCanvas / 2 + 'px)')
              canvas.css('left', 0)
              console.log('Шире This.hCanvasThis.wCanvas  w '+This.wCanvas  + " h "+ This.hCanvas)
            }

            // var realWidth3 = This.hCanvas *(displayWidth/This.hCanvas);
            // var realWidth4 = This.hCanvas *(16 / 9);
            // var realHeight2 = This.hCanvas;
            //
            //
            // console.log('testRealVideo2  w '+realWidth2 + " h "+ realHeight2)
            // console.log('testRealVideo3  w '+realWidth3 + " h "+ realHeight2)
            // console.log('testRealVideo4  w '+realWidth4 + " h "+ realHeight2)
            ...
            console.log('__________startFuncOperOther__4____________ ')
            //console.log('then __')
          }, 1000)
        }
        else {
          console.error('mainVideoPosition false '+senderId)
        }
      }, 200)
    },
    ...
    closeAllFuncOper(message) {
      $('#participants').css('overflow-y', 'auto')
      this.isShowFuncOther = false
      This.updateMainOperIG(1)
      ...
      console.log('_________________________ ')
      console.log('_________________________ ')
      console.log('_______participant.isRealFuncOper = false__________________ ', senderId)
      if (this.mainWin > 1) {
    ...
    moveDrawMess(message) {
      ...
      if (message.status === 'on') {
        ...
        if (this.otherX == -100 && this.otherY == -100) {
          ...
          this.otherCTX.lineWidth = message.pointer_width/2; // толщина линии
    ...
    arrowDrawMess(message) {
      ...
      if (message.type == 1) {
        ...
        This.drawArrowNew(this.otherCTX, message.x1 * this.wCanvas, message.y1 * this.hCanvas, message.x2 * this.wCanvas, message.y2 * this.hCanvas, message.pointer_width/2, message.pointer_color)
      } else {
...
//region Для тестирования
    ...
    testReceiveVideo(senderName, _senderIdtest, isBodyPix, isMic, isVideo, isFuncOper) {
      ...
      var video12 = `<div id="${'devVideo_' + senderIdtest}" class="winVideoRoom1">
                          <div id="${'devDevIconNoVideo_' + senderIdtest}" class="devDevIconNoVideoPh">
...
<!--                          <span <div id="${'spanNoVideo' + senderIdtest}" class="fa fas fa-video-slash v-btn_ot_stats_sp ma-2" style="color: white; font-size: 60px" aria-hidden="true"></span>-->
  ...
  <!--                           <span class="fa fas fa-microphone-alt v-btn_ot_stats_sp ma-2" aria-hidden="true"></span>-->
    ...
    setPanelInfoUser(b) {
      console.log('setPanelInfoUser '+b)
      if (b == false) {
        this.isPanelInfoUser = false
        console.log('setPanelInfoUser 1')
      } else if (b == true) {
        this.isPanelInfoUser = true
        console.log('setPanelInfoUser 2')
      } else {
        this.isPanelInfoUser = !this.isPanelInfoUser
        console.log('setPanelInfoUser 3')
      }
      ...
      if (this.isPanelInfoUser) {
        ...
        document.getElementsByClassName('v-expansion-panel-content__wrap')[0].classList.add('listUserPH');
      ...
      if (parsedMessage.wav) {
        ...
        var Sound = (function () {
          ...
          return function Sound(src) {
            ...
            setTimeout(() => {
              snd.volume = 1
              // console.log('Audio 4 audioTracks', snd.audioTracks.length)
              // console.log('Audio 4 audioTracks', snd.audioTracks)
              // console.log('Audio 4 audioTracks', snd.audioTracks[0])
    ...
    asrtTranlsateResultAudio(parsedMessage) {
      ...
      if (This.isAudioPlayATSender == 0) {
        ...
        //
        //     if (sender.track.kind == 'audio') {
        //       console.log('_+_+_SAVE AUDIO_______ .track')
        //       trackAudio = sender.track
        //
        //       var Sound = (function () {
        //
        //         var df = document.createDocumentFragment();
        ...
        //
        //             // var OutgoingAudioMediaStream = new MediaStream();
        //             // OutgoingAudioMediaStream.addTrack(trackAudio);
        //             //
        //             // var IncomingAudioMediaStream = new MediaStream();
        //             // IncomingAudioMediaStream.addTrack(snd.audioTracks[0]);
        //             //
        //             // const audioContext = new AudioContext();
        //             //
        //             // var audioIn_01 = audioContext.createMediaStreamSource(OutgoingAudioMediaStream);
    ...
    addWinTest(){
      This.devTestSession = true
      This.testUserWin++
      This.testReceiveVideo('testWinVideo' + This.testUserWin, 4000+This.testUserWin, false, false, true, false)
    },

    updateMainOperIG(t){
      console.log('updateMainOperIG '+t)
      setTimeout(()=>{
        var keys = Object.keys(stream.calls);
        for (var i = 0; i < keys.length; ++i) {
          var participant = stream.calls['' + keys[i]];
          if (participant.OperFunc) {
            This.mainVideoPosition(participant.senderId)
            break;
          }
        }}, 1000)
    },
    ...
    swipeOper(isLeft){
      ...
      setTimeout(()=>{
        ...
        for (var i = 0; i < keys.length; ++i) {
          var participant = stream.calls['' + keys[i]];
          if (participant.OperFunc || participant.scrs) {
            opers.push(participant.senderId)
          }
        }
        // console.log('opers',opers)
        // console.log('opers.length',opers.length)

        var senderIdNew = 0

        if(opers.length > 1){
          for(var f = 0; f < opers.length; ++f){
            var participant_obj = stream.calls['' + opers[f]]
            if(participant_obj.full) {
              // console.log('participant_obj.full '+opers[f])
              if(isLeft){
                if(f == opers.length-1){
                  senderIdNew = opers[0]
                }else{
                  senderIdNew = opers[f+1]
                }
              }
              else{
                if(f == 0){
                  senderIdNew = opers[opers.length-1]
                }else{
                  senderIdNew = opers[f-1]
                }
              }
              // console.log('senderIdNew',senderIdNew)
              This.mainVideoPosition(senderIdNew)
              break;
            }
          }
        }
...
.imgDevIconNoVideoTextPhDevMainMin {
  width: 30px;
  height: 30px;
  border-radius: 84px !important;;
  background: #6ba5ff !important;;
...
button.mr-1.btnFooter3.devBtnGC.v-btn.v-btn--is-elevated.v-btn--fab.v-btn--has-bg.v-btn--round.theme--light.v-size--default.white {
